...
export function deriveSearchParams(houseProfile) {
  ...
  const neighborhoodKeyword = ext.neighborhoodChar
    ? [NEIGHBORHOOD_KEYWORDS[ext.neighborhoodChar]].filter(Boolean)
    : [];

  const allKeywords = [...mustHaveKeywords, ...neighborhoodKeyword];
