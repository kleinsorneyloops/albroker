...
export async function searchListings(params = {}) {
  ...
  if (views)               apiParams.views      = views;
  if (sqft)                apiParams.sqft       = sqft;
  if (yearBuilt)           apiParams.yearBuilt  = yearBuilt;
  if (lotSize)             apiParams.lotSize    = lotSize;
  if (hasHOA !== undefined && hasHOA !== null) apiParams.hasHOA = hasHOA;
...
export function normalizeProperty(item) {
  ...
  return {
    ...
    // resoFacts — only present on zpid detail responses
    // Flattened for direct scoring access
    resoFacts,
    hasFireplace:     resoFacts?.hasFireplace ?? null,
    hasGarage:        resoFacts?.hasGarage ?? null,
    hasCooling:       resoFacts?.hasCooling ?? null,
    hasHeating:       resoFacts?.hasHeating ?? null,
    hasView:          resoFacts?.hasView ?? null,
    hasWaterfront:    resoFacts?.hasWaterfrontView ?? null,
    hasPool:          resoFacts?.hasPrivatePool ?? null,
    hasAssociation:   resoFacts?.hasAssociation ?? null,
    monthlyHoaFee:    resoFacts?.associationFee ?? p.monthlyHoaFee ?? null,
    parkingSpaces:    resoFacts?.coveredParkingCapacity ?? resoFacts?.garageParkingCapacity ?? null,
    fireplaceCount:   resoFacts?.fireplaces ?? null,
