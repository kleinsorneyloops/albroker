...
export default async (req) => {
  ...
  if (req.method === 'POST') {
    try {
      ...
      // If no userId, create a new user row first
      const userId = existingId || randomUUID();

      if (!existingId) {
        // Insert a minimal user record — Better Auth will enrich this later
        await sql`
  ...
  if (req.method === 'GET') {
    ...
    try {
      const rows = await sql`
        SELECT quiz_answers, life_stage, timeline, school_importance, profile_completeness, last_updated
...
// Estimate profile completeness based on how many fields are filled
...
export default async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      // quiz_answers stores the full profile object — return it directly
      // so the existing frontend (onboard-flow.jsx, profile/page.jsx) keeps working
      const profile = rows[0].quiz_answers || {};
